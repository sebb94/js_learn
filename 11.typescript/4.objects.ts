...
function printName(person: { first: string; last: string }): void {
  ...
  function doublePoint2(point: { x: number; y: number }): {
    x: number;
    y: number;
  } {
  ...
  let coordinate: Point2 = { x: 34, y: 2 };
  
  function randomCoordinate(): Point2 {
    return { x: Math.random(), y: Math.random() };
  }
  
  function doublePoint(point: Point2): Point2 {
    return { x: point.x * 2, y: point.y * 2 };
  }
